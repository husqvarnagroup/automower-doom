  ...
  int       ticks )
{
    ...
    {
	// Draw game ticks
	x = WI_drawNum(x, y, ticks, 2) - SHORT(colon->width);
...
void WI_updateStats(void)
{
	...
	cnt_ticks = plrs[me].stime % TICRATE;
...
void WI_drawStats(void)
{
    ...
    WI_drawTime(SCREENWIDTH/2 - SP_TIMEX, SP_TIMEY, cnt_time, cnt_ticks);

    if (wbs->epsd < 3)
    {
	V_DrawPatch(SCREENWIDTH/2 + SP_TIMEX, SP_TIMEY, par);
	WI_drawTime(SCREENWIDTH - SP_TIMEX, SP_TIMEY, cnt_par, 0);
    }
